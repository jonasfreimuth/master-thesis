...
[Disentangle terminology of deconvolution / expression purification.]
...
[@lahdesmaki.2005]. Historically, these methods were based on non-negative
matrix factorization (NMF), which modeled the problem as consisting of
...
genes particularly highly expressed in specific cell types. This approach was
then refined into a multi-step approach
...
Most older methods addressing the issue of disentangling tumor expression from
other cells, like DeMix [@ahn.2013], ISOpure [@aran.2017], were designed for
expression profiling arrays and may not generalize to the unlimited number of
genes made accessible by bulk RNA-seq. Also, they were designed to only
distinguish between two or few cell types / conditions and may not be suited to
dissect the cancer cells from the specific mix of cell types surrounding them.
