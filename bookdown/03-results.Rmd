...
import::here("stringr", "str_to_sentence")
...
plot_data_paths <- plot_handles$plot_data_paths
table_caps <- plot_handles$table_caps
```
...
rnd_sim_data %>%
  ...
  arrange(heterogeneous_cancer, split, tech_noise_level) %>%
  kbl(
...
the models trained on my method were able to predict PFI with higher accuracy
than the model trained on raw bulk data with the same gene set, but not better
than models trained on raw bulk data with other gene sets. While my method
