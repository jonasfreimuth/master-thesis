...
to a distinct tumor sample in the original data. A subset of 24 tumor samples
from the dataset by @wu.2021 has bulk RNA-seq data available, allowing me to
directly compare performance on pseudo-bulk and real-world bulk RNA-seq data. Of
those, only 18 included cells annotated as belonging to the cancer cell type.
For comparability, only those 18 samples were then used for both bulk and
pseudo-bulk data, see Supp. Table \@ref(tab:bp-sample-data) for which samples
were used. To compute references, I used the same set of thresholds on the
marker genes and contrasted across marker selection methods, but did not include
the contrasting across other reference parameters to simplify the analysis. For
these other parameters, I used the values expected to lead to optimal results,
namely including cancer splitting, and marker gene quality control. Lastly, I
deconvoluted every bulk and pseudo-bulk using every reference as before, except
using both deconvPure and BayesPrism.
...
results into context, I also ran BayesPrism on the same data. However, as
