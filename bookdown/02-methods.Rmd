...
matrix from @wu.2021, stratified by cell type. Cell type fractions $\mathbf{f}$
were also randomly selected by first choosing each cell type's fraction
uniformly at random, and then normalizing the fraction of all non-cancer cell
types to the complement of the fraction of the cancer cell type:

\begin{equation*}
  \begin{aligned}
    f_{\text{cancer}}           &\sim \mathcal{U}(0, 1) \\
    f_{\text{non cancer, raw}}  &\sim \mathcal{U}(0, 1 - f_{\text{cancer}}) \\
    \left( f_{\text{non cancer, norm}} \right)_j &= \frac{
      \left( f_{\text{non cancer, raw}} \right)_j
    } {
      \sum_{j=1}^{m-1} {f_{\text{non cancer, raw}}}
    }
  \end{aligned}
\end{equation*}
...
\begin{equation*}
  \mathbf{f} = \left( \begin{array}{c|c}
    f_{\text{cancer}} &
    \left( f_{\text{non cancer, norm}} \right)_1 &
    \left( f_{\text{non cancer, norm}} \right)_2 &
    \cdots &
    \left( f_{\text{non cancer, norm}} \right)_{m - 1}
  \end{array} \right)
\end{equation*}
...
Of the three high resolution *in-silico* dissection methods mentioned before, I
chose BayesPrism for comparison here since it is specifically designed for tumor
