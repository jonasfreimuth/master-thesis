...
inclusion of the cancer cell type reference profile (Figure
\@ref(fig:workflow)).
...
cell type and scaling it up to the number of cells $z$ required.
...
cell type expression profile was included. Note however, that a thresholds for
the different marker selection methods result in marker sets of different sizes
as a threshold of one (all markers) for Wilcoxon based selection implies all
genes with a $p$-value below 0.05, but for random markers it implies all genes.
...
were again randomized using the same prodcedure as during
[random simulations](#normally-distributed-simulation). I then deconvoluted each
pseudo-bulk using each reference and calculated the average Pearson's $\rho$ and
its 95% CI between true and estimated values of cancer cell type expression
across all deconvolutions with the same set of parameters.

## Evaluation

After exploring the behavor of deconvPure under ideal conditions, I compared its
...
(PFI) and compared the performance of both BayesPrism and deconvPure to models
trained on raw bulk data.

### Comparison to BayesPrism

<!-- TODO Characterize real-world bulk data. Add table explaining cancer content
-->
...
analysis framework since both are written in `R`. Here I use the `InstaPrism`
implementation [@hu.2023] providing substantial speedup compared to the
original.
...
RNA-seq data. Of those, only 18 included cells annotated as belonging to the
cancer cell type. For comparability, only those 18 samples were then used for
both bulk and pseudo-bulk data. To compute references, I used the same set of
thresholds on the marker genes and contrasted across marker selection methods,
but did not include the contrasting across other reference parameters to
simplify the analysis. For these other parameters, I used the values expected to
lead to optimal results, namely including cancer splitting, and marker gene
quality control. Lastly, I deconvoluted every bulk and pseudo-bulk using every
reference as before, except using both deconvPure and BayesPrism.

I extracted and processed residuals from deconvPure the same as in the scRNA-seq
...
genes, so comparisons between it and deconvPure are based on a different number
of genes (all shared genes between reference and bulk for deconvPure, all marker
genes for BayesPrism). As before, I used the average Pearson's $\rho$ with 95%
CI across deconvolutions of the same parameters to assess the relationship of
predicted expression to actual expression. I also carried out the same analysis
of the benefit of expression estimation over the inherent information on cancer
expression in the bulk across cancer cell proportions.
...
of a set 50 genes. The PFI on the other hand, is a property of the patients'
disease progression. Compared to the overall survival of the patient which may
be of greater interest, it has the advantage of greater availability of data due
to shorter followup times [@liu.2018]. As predicting a clinical outcome from
expression data alone is bound to result in low accuracy, I included the age and
tumor status at diagnosis in the prediction model along with the expression data
during model training. To ensure consistent data, I restricted analysis to
...
recorded. This left `r txt_pr_srv_stat("n_sample")` samples with PFI data and
data on the clinical variables mentioned above.
...
the previous [comparison](#comparison-to-bayesprism), except without doing any
marker selection, as the comparison sections showed no benefit to estimation
accuracy. (I still include results generated with different marker selection
methods and thresholds in the supplementary materal, Supp. Figures
\@ref(fig:pred-cat-perf-supp-plt) & \@ref(fig:pred-srv-perf-supp-plt), Supp.
Tables \@ref(tab:pred-cat-perf-tab) & \@ref(tab:pred-srv-perf-tab)). To put the
results into context, I also ran BayesPrism on the same data again. However, as
BayesPrism is not able to estimate expression for non-marker genes, there is a
possibility that differences in performances of the models trained on the
expression estimates are confounded by the differences in the number of genes.
To address this, I also trained models on bulk data with gene sets restricted to
each set of genes from the cancer expression estimates.
