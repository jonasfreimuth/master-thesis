...
<!-- TODO Talk about zero transript prediction mitigation. -->
...
To demonstrate the principle of the method, I first generated model
expression profiles, simulated measurements of expression quantification, and
analyzed the behavior of my method on it.

<!-- TODO Precisely describe sampled distributions. -->

The simulated expression profiles were generated by sampling expression data for
XXX cell types & XXX transcripts from a normal distribution. For each set of
expression profiles, I then simulated the result of a bulk RNA-seq experiment,
essentially by running cell type deconvolution in reverse. I first sampled cell
type counts for all cell types uniformly at random, and then summed the
simulated expression profiles after multiplying each by its cell type's count.
To each simulated bulk expression vector I then added simulated technical noise
at different intensities by sampling each from a normal distribution with
varying standard deviations.

<!-- TODO Describe how cancer expression heterogeneity was modeled.
Currently, this is done incorrectly by just giving the *reference profile* a
higher SD. What should happen is that more noise is added to the reference
profile before it is added to the bulk. -->

I then the resulting set of simulated bulks with associated reference expression
profiles deconvoluted, each with the reference profiles either including or
excluding the designated cancer cell type. I analyzed the distribution of
residuals when compared to the true expression profile of the cancerous cell
type using Pearson's $\rho$ and compared the performance across the type of
reference used, the relative level of technical noise added, and whether the
cancerous cell type was modeled as having a more heterogeneous expression
profile than other cell types. Also, I examined the effect of these parameters
on the root mean squared error (RMSE) between true and predicted cell type
counts.
...
abundances and expression profiles. I used single cell RNA sequencing data
consisting of transcript count data breast cancer tumor samples from @wu.2021
...
Since the dataset provided by @wu.2021 is already filtered for high quality
cells, I performed no additional cell quality control.
...
choosing a random value for the marker score to ensure any changes in
performance were not simply due to the restricted size of the set of marker
genes.

I computed reference profiles across a spectrum of parameters, one reference for
each combination. For marker selection strategies I used both methods outlined
above (Wilcoxon test, random score), each with a threshold of the top XXX, XXX,
..., of the best scoring marker genes being selected. I also contrasted across
including the cancer cell type or not, and including either only genes passing
transcript quality control or all marker genes for a reference.
...
pseudo-bulks, and to actual bulk RNA-seq samples, both from @wu.2021. Of the
three high resolution *in-silico* dissection methods mentioned before, I chose
BayesPrism for comparison here since it is specifically designed for tumor
samples, and integrates well with my existing analysis framework since it is
also written in `R`.

<!-- TODO Ensure it is clear how both cell type count and cancer expression
predictions were extracted from BayesPrism. -->
...
included the expected optimal parameters, namely that cancer splitting, Wilcoxon
marker selection, and marker gene quality control would lead to the best
results.
...
extracted and processed residuals from NNLS as in the scRNA-seq based
pseudo-bulks [section](#scrna-seq-pseudo-bulks).
...
The data comprised all 1'095 patient samples from the TCGA-BRCA project with
bulk RNA-seq data available . For the survival prediction task, I retrieved data
on survival endpoints derived from curated clinical data from @liu.2018 for the
