...
\@ref(tab:rnd-sim-ratios) for all parameters). The simulated expression profiles
...
cell type. I analyzed the effect of these parameters on the estimation accuracy
of cell type counts via the inverse root mean squared error ($1 / RMSE$) between
true and predicted cell type counts. Lastly, I examined the relationship between
the distribution of residuals to the true expression profile of the cancerous
cell type using Pearson's $\rho$. I repeated this `r txt_rs_stc("n_repeats")`
times for each level of technical noise and averaged results across replicates.
...
gene in the deconvolution reference. Genes with a good score would have an
...
average performance metrics across all deconvolutions with the same set of
parameters. These were again the inverse root mean squared error ($1 /
\text(RMSE)$) between true and estimated cell type concentrations, and average
Pearson's $\rho$ between true and estimated values cancer cell type expression.
...
between it and my method are based on a different number of genes. As before, I
used the average Pearson's $\rho$ across deconvolutions with the same
parameters to assess the relationship of predicted expression to actual
expression.
...
patient level, I restricted the bulk data to one sample per patient, choosing
the first sample recorded. This left `r txt_pr_srv_stat("n_sample")` samples
with PFI data.
...
of models trained on cancer expression estimations from either method. As
...
expression estimated are due to the differences in the amount of genes. To
...
I transformed each feature set (i.e., raw bulk and cancer expression) onto a
logarithmic scale ($\mathbf{x}_{\text{trans}} = \log_{10}{\left( \mathbf{x} + 1
\right)}$, only if it was not already on that scale, as was the case for
