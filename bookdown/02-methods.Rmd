...
of one to preserve values of zero on the original scale) so that the variance in
residuals stays reasonably consistent across the spectrum of actual and
predicted gene expression values.

<!-- TODO Talk about zero transript prediction mitigation. "Also, some cell types
may erroneously be predicted as absent from a bulk (i.e. at an abundance of
zero)..." -->
...
$\mathbf{S}_{sim}$ were generated by sampling expression data for $n = XXX$
transcripts & $m = XXX$ cell types from a normal distribution:
...
control, during which I filtered genes to be expressed in at least XXX cells,
removed genes which could potentially introduce batch effects, and removed
non-protein coding genes. In this, I generally followed the methodology from
@chu.2022, except I did not remove genes from sex chromosomes as these might
play a role in breast cancer. Also, I assigned each gene a marker score,
quantifying its likely usefulness as a gene in the deconvolution reference.
Genes with a good score would have an expression signature across cell types
which would allow for clear identification of just a single cell type by having
particularly high or low expression in just that cell type. I explored multiple
ways to determine this score during preliminary experiments [DESeq2 @love.2014;
Hampel filter @davies.1993], but ultimately chose a simple Wilcoxon test
...
selecting the top XXX, XXX, ..., of the best scoring marker genes. I also
contrasted across including the cancer cell type or not, and including either
only genes passing transcript quality control or all marker genes for a
reference.

I generated XXX pseudo-bulks by randomly sampling XXX cells (twice the median
number of cells per sample in the data) with replacement from the overall count
matrix from @wu.2021, stratified by cell type. Cell type fractions
$\mathbf{f}_norm$ were also randomly selected by first choosing each cell type's
fraction uniformly at random, and then normalizing the fraction of all
non-cancer cell types to sum to the complement of the fraction of the cancer
cell type:
...
expression predictions for all cell types on the original scale. As before, I
...
Since the PFI is a variable on the patient level, I restricted the bulk data to
only solid tissue tumor samples (no non-tumor or metastatic samples), and for
each patient chose the first sample recorded. This left me with XXX tumor
samples, each corresponding to a unique patient.

First, I applied my method to all bulk data samples by deconvoluting it using
references derived from the @wu.2021 scRNA-seq data like in the previous
sections, except with a smaller set of thresholds (XXX) on marker gene scores.
Like in the [section](#comparison-to-bayesprism) comparing my method to
BayesPrism, I also ran the latter method on the same data to later compare the
prediction accuracy of models trained on cancer expression estimations from
either method. To add more context to the observed differences in the
performance the models trained on bulk data and estimated cancer expression, I
also trained models using the estimated combined TME expression. For my method,
this was simply the fitted values of the deconvolution model
($\mathbf{\hat{b}}$, see the
...
at a ratio of 7:3. Using the preprocessing facilities of the `caret` package
[@kuhn.2008], I performed removal of zero-variance genes, centering, scaling,
and finally dimensionality reduction via principal component analysis (at a
threshold of 95% of variance explained for including components) on the training
set. The same procedure was applied to the testing data using the same
parameters for each step as for training data to avoid information leakage.
...
were trained using XXX-fold cross-validation, repeated XXX times. I used the
"Synthetic Minority Oversampling Technique" [SMOTE, @chawla.2002] for resampling
during each training iteration to ensure balanced representation of classes.
...
I trained random survival forest models with XXX trees by first performing
hyperparameter tuning for finding the optimal parameters for the data. These
were the number of randomly chosen variables for splitting at each node
(parameter `mtry`), and the minimal number of cases that a terminal node of a
tree may contain (parameter `nodesize`). The performance for random survival
forests is measured using the C-index [@harrell.1982], comparing the observed
ordering of events taking place to the predicted.
...
using half the size of the test data for each evaluation dataset and repeating
the process 200 times.
